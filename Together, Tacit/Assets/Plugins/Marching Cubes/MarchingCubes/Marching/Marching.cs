...
namespace MarchingCubesProject
{
    public abstract class Marching : IMarching
    {
        ...
        public virtual void Generate(IList<float> voxels, int width, int height, int depth, IList<Vector3> verts, IList<int> indices, float scale, float heightOffset)
        {
            ...
            for (x = 0; x < width - 1; x++)
            {
                for (y = 0; y < height - 1; y++)
                {
                    for (z = 0; z < depth - 1; z++)
                    {
                        ...
                        //Perform algorithm
                        March(x, y, z, Cube, verts, indices, scale, heightOffset);
                    }
                }
            }
        ...
        /// </summary>
        protected abstract void March(float x, float y, float z, float[] cube, IList<Vector3> vertList, IList<int> indexList, float scale = 1.0f, float heightOffset = 1.0f);
